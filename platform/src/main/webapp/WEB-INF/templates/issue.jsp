...
<script src="/static/js/commons/mixins.js" type="text/javascript" ></script>
</head>
<body>
<div class="wrapper"> 
	...
	<el-dialog :title="readFormTtile" v-model="readFormVisible">
		...
		<el-table v-if="!readFormEditable" :data="issueRecords" style="width: 100%">
			<el-table-column
			type="index"
			width="40">
		</el-table-column>
		...
		<el-button v-if="!readFormEditable" @click.native="handleAddRecord">评论</el-button>
...
<script>
 ...
 mounted(){
	let arr = eval('${issues}');
	sendRequest("/api/issues/", "GET")
	.then(resp=>{
		let issues = resp._embedded.issues;
		issues.forEach(item => {
			item.date = moment(item.date)._d;
		});
		this.tableData = issues;
	});
 ...
 methods:{
	...
	handleSearch(){
		let _this = this;
		let url = "/api/issues/search/findByTitleLike";
		let data = _this.formInline;
	...
	handleRead: function(row){
		...
		let url = row._links.issueRecord.href;
		sendRequest(url, "GET")
		.then(resp=>{
	...
	handleSubmit(){
		...
		let _this = this;
		sendRequest(url, method, this.readForm)
		.then(resp=>{
	...
	handleDelete(row){
		let _this = this;
		sendRequest("/deleteIssue/", "POST", row)
		.then(resp=>{
			_this.readFormVisible = false;
			_this.handleSearch();
		});
	...
	//评论
	handleAddRecord(){
		this.issueRecordFormVisible = true;
		this.issueRecordForm.content = "";
	},
	handleRecordSubmit(){
		let url = "/issueRecord";
		let _this = this;
		let data = _this.issueRecordForm;
		data.name = '${CURRENT_USER.username}';
		data.issueId = _this.readForm.id;
		sendRequest(url, "POST", data)
		.then(resp=>{
			_this.issueRecordFormVisible =false;
			_this.issueRecords.push(data);
		});
	},
}
})
